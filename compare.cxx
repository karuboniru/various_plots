...
int main(int argc, char *argv[])
{
    ...
    if (argc < 4)
    {
        std::cout << "Usage: " << argv[0] << " <input_file_list_res2> <input_file_list_hyb> <output_prefix> [do_cut (default off) 0=off, 1=on]" << std::endl;
        return 1;
    }
    ...
    std::unordered_map<std::string, double> xsecs{}, xsecs_hyb{};
    int Q_bin_count{80}, W_bin_count{70}, E_bin_count{80};
    double Q_min{0}, Q_max{1.8}, W_min{1.00}, W_max{1.70}, E_min{0}, E_max{10};
    {
    ...
    for (const auto &[channelname, title] : single_pion_channels)
    {
        ...
        auto xsecp = std::make_pair(xsecs[channelname], xsecs_hyb[channelname]);
        plot_compare(Qhistograms[channelname].get(), Qhistograms_hyb[channelname].get(), output_prefix, xsecp);
        plot_compare(Whistograms[channelname].get(), Whistograms_hyb[channelname].get(), output_prefix, xsecp);
        plot_compare(Ehistograms[channelname].get(), Ehistograms_hyb[channelname].get(), output_prefix, {0, 0});
    }
