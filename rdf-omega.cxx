...
int main(int argc, const char **argv) {
  ...
  auto dataset =
      d.Define(
          ...
          .Define("omega_P",
                  [](event &e) {
                    return e.get_particle_nofsi(223).begin()->second;
                  },
                  {"event"})
          .Define("pomega", [](TLorentzVector &p) { return p.P(); },
                  {"omega_P"})
          .Define("ppion", [](TLorentzVector &p) { return p.P(); },
                  {"sum_p_pion_system"})
          .Define("momega", [](TLorentzVector &p) { return p.M(); },
                  {"omega_P"})
          .Define("channelname",
                  [](event &e) { return e.get_channelname_no_nucleon(); },
                  {"event"})
          .Define("gamma_count", [](event &e) { return e.count_out(22); },
                  {"event"});
  ...
  auto dataset_3pi =
      dataset
          ...
          .Define("pip_count", [](event &e) { return e.count_out(211); },
                  {"event"})
          .Define("deltaP",
                  [](TLorentzVector &sum_p, TLorentzVector &omega_P) {
                    return (sum_p - omega_P).P();
                  },
                  {"sum_p_pion_system", "omega_P"})
          .Define("deltaM",
                  [](TLorentzVector &sum_p, TLorentzVector &omega_P) {
                    return sum_p.M() - omega_P.M();
                  },
                  {"sum_p_pion_system", "omega_P"});

  auto dataset_gamma_pi =
      dataset
          .Filter(
              [](size_t gamma_count, int pion_count) {
                return (gamma_count == 1) && pion_count;
              },
              {"gamma_count", "pion_count"})
          .Define("gamma_P", [](event &e) { return e.get_leading_out(22); },
                  {"event"})
          .Define("W_gamma_pi_system",
                  [](TLorentzVector &sum_p, TLorentzVector &gamma_P) {
                    return (sum_p + gamma_P).M();
                  },
                  {"sum_p_pion_system", "gamma_P"});
  ...
  objs_list
      ...
      ->Scale(1. / count, "WIDTH");
  objs_list
      .emplace_back(dataset_gamma_pi.Histo1D(
          {"W_gammapi",
           "#it{W} of #gamma #pi system, 2#pi cut; #it{W} "
           "(GeV); Count (1/GeV)",
           128u, 0.4, 0.9},
          "W_gamma_pi_system"))
      ->Scale(1. / count, "WIDTH");
  objs_list
      .emplace_back(
          dataset_gamma_pi.Filter("pion_count == 1")
              .Histo1D({"W_gammapi_1pi",
                        "#it{W} of #gamma #pi system, 2#pi cut; #it{W} "
                        "(GeV); Count (1/GeV)",
                        128u, 0.4, 0.9},
                       "W_gamma_pi_system"))
  ...
  objs_list
      .emplace_back(dataset.Histo1D(
          {"pomega", "; #it{p_{#omega}} (GeV/#it{c}); Count (1/GeV/#it{c})",
           128u, 0, 0},
          "pomega"))
      ->Scale(1. / count, "WIDTH");
  objs_list.emplace_back(dataset_3pi.Histo1D("ppion"))
      ->Scale(1. / count, "WIDTH");
  objs_list.emplace_back(dataset_3pi.Histo1D("momega"))
      ->Scale(1. / count, "WIDTH");
  ...
  objs_list
      .emplace_back(event_pi_nofsi.Histo1D(
          {"pomega_pi_nofsi",
           "; #it{p}_{#omega} (GeV/#it{c}); Count (1/GeV/#it{c})", 128u, 0, 0},
          "pomega"))
      ->Scale(1. / count, "WIDTH");
  objs_list
      .emplace_back(
          event_pi_nofsi.Filter("deltaP < 3e-5")
              .Histo1D({"pomega_pi_nofsi_omega_nofsi",
                        "; #it{p}_{#omega} (GeV/#it{c}); Count (1/GeV/#it{c})",
                        128u, 0, 0},
                       "pomega"))
      ->Scale(1. / count, "WIDTH");
  objs_list
