...
int main(int argc, const char **argv) {
  if (argc == 1)
    return 1;
  auto file = std::make_unique<TFile>("output_nofsi_1pi.root", "RECREATE");
  ...
  auto dataset_cut =
      dataset
          ...
          .Filter(
              [](event &e) {
                // return e.count_out(211) + e.count_out(-211) == 1 && e.count_out(111) == 0 && e.count_out(2212) + e.count_out(2112) == 1;
                return e.count_out(211)  == 1 && e.get_out_count() == 3;
              },
              {"event"})
          // W_rest < 1.4
          .Filter(
  ...
  objs_list
      ...
      ->Scale(xsec / count * 1e3, "WIDTH");
  auto &&h_theta_nobinning = objs_list.back();
  std::fstream filetxt("out_1pi.txt", std::ios_base::trunc | std::ios_base::out);
  for (int i = 0; i < h_theta_nobinning->GetNbinsX(); ++i) {
    filetxt << h_theta_nobinning->GetBinCenter(i + 1) << " "
            << h_theta_nobinning->GetBinContent(i + 1)
            << " "
            // << h_theta_nobinning->GetBinError(i + 1)
            << std::endl;
  }
      ...
      .emplace_back(
          dataset_cut.Filter("StdHepPdg[1] == 2212")
              .Histo1D({"h_Tk_nobinning_H", "h_Tk_nobinning_H", 500, 0, 700.},
                       "pion_Tk"))
      ->Scale(xsec / count * 1e3 * 13, "WIDTH");
  auto && h_Tk_nobinning_H = objs_list.back();
  std::fstream filetxt_H("out_1pi_H.txt",
                         std::ios_base::trunc | std::ios_base::out);
  for (int i = 0; i < h_Tk_nobinning_H->GetNbinsX(); ++i) {
    filetxt_H << h_Tk_nobinning_H->GetBinCenter(i + 1) << " "
              << h_Tk_nobinning_H->GetBinContent(i + 1)
              << " "
              // << h_Tk_nobinning_H->GetBinError(i + 1)
              << std::endl;
  }
